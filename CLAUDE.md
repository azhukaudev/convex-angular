...
- Single library test file: `nx test convex-angular -- inject-query` — everything after `--` is forwarded to `jest`, and a bare argument is a test-path pattern (`--testPathPatterns=inject-query` works too; Jest 30 renamed the singular `--testPathPattern` and rejects it). Or run Jest directly: `cd packages/convex-angular && pnpm exec jest inject-query`.
- Filter by test name: `nx test convex-angular -- -t "skipToken"` (or `cd packages/convex-angular && pnpm exec jest -t "skipToken"`)
...
- **Localized change** (one or two files, no API surface change): run the relevant targeted test — `nx test convex-angular -- <path-pattern>` or `nx test convex-angular -- -t "<name>"` — then `pnpm verify:quick`.
...
- **Node version**: `.nvmrc` pins Node 24 (CI reads it via `node-version-file`); `nvm use` before testing/pushing. The Node 24 jest-worker SIGSEGV is real and Jest 30 did not fix it — `nx run-many -t test` segfaulted in 2 of 8 runs at Jest's default worker count, and in 0 of 25 at `--maxWorkers=4`; Node 22 never reproduced it. Both `jest.config.cjs` files therefore set `maxWorkers: 4`, which costs no speed (worker startup dominates these small suites) and is near a no-op on 4-core CI runners. Treat it as a probability reduction, not a fix: if a heavier spec resurfaces SIGSEGV, lower `maxWorkers` further or re-pin `.nvmrc` to 22.
...
- Tests are Jest via `jest-preset-angular` (`createCjsPreset()`, zone-based TestBed through `setupZoneTestEnv()`, `fakeAsync` supported); specs live next to source as `*.spec.ts`. Each project has its own `jest.config.cjs` (deliberately `.cjs`, not `.ts` — the file's header comment explains why) and `src/test-setup.ts`. Neither `project.json` declares a `test` target: Nx infers it from the jest config via the `@nx/jest/plugin` entry in `nx.json`, so the target is `nx:run-commands` running bare `jest` with `cwd` set to the project root.
